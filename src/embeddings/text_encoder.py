...
from src.utils.config import EMBEDDING_DIM, TEXT_EMBEDDING_DIM, TEXT_MODEL_NAME
...
class TextEncoder:
    ...
    def __init__(self, model_name: str = TEXT_MODEL_NAME):
        ...
        self._proj_matrix = np.zeros(
            (TEXT_EMBEDDING_DIM, EMBEDDING_DIM), dtype=np.float32
        )
        self._proj_matrix[:, :TEXT_EMBEDDING_DIM] = np.eye(TEXT_EMBEDDING_DIM)

    def _text_from_listing(
        self, title: str, product_type: str, color: str, material: str
    ) -> str:
